...
namespace OpenMetaverse.Rendering
{
    public partial class LindenRenderer : IRendering
    {
        ...
        private static void CreateSide(ref Face face, Primitive prim, List<Vertex> primVertices, Path path,
            Profile profile, bool isSculpted)
        {
            ...
            if (!isSculpted)
            {
                ...
                if (prim.PrimData.PathCurve == PathCurve.Circle &&
                    prim.PrimData.ProfileCurve == ProfileCurve.HalfCircle)
                {
                    if (sBottomConverges)
                    {
            ...
            else
            {
		        ...
		        if (average_poles)
		        {
			        ...
			        for (i = 0; i < face.NumS; i++)
				        average += face.Vertices[i].Normal;
                    ...
                    for (i = 0; i < face.NumS; i++)
                        average += face.Vertices[i + face.NumS * (face.NumT - 1)].Normal;

			        // Set average
                    for (i = 0; i < face.NumS; i++)
                    {
                        Vertex v = face.Vertices[i + face.NumS * (face.NumT - 1)];
                        v.Normal = average;
                        face.Vertices[i + face.NumS * (face.NumT - 1)] = v;
                    }
		        }
		        ...
		        if (wrap_s)
		        {
			        for (i = 0; i < face.NumT; i++)
			        {
                        ...
                        Vertex v = face.Vertices[face.NumS * i];
                        v.Normal = norm;
                        face.Vertices[face.NumS * i] = v;

                        v = face.Vertices[face.NumS * i + face.NumS - 1];
                        v.Normal = norm;
                        face.Vertices[face.NumS * i + face.NumS - 1] = v;
			        }
		        }
		        ...
		        if (wrap_t)
		        {
                    for (i = 0; i < face.NumS; i++)
			        {
                        ...
                        v = face.Vertices[face.NumS * (face.NumT - 1) + i];
                        v.Normal = norm;
                        face.Vertices[face.NumS * (face.NumT - 1) + i] = v;
			        }
		        }
            }
